...
from model import AdversarialModel, Encoder, TrainModel
from utils import calculate_weighted_homophily, get_dataset, set_everything
...
if __name__ == '__main__':
    ...
    base_model_map = {'GCNConv': GCNConv}
    base_model = base_model_map[args.base_model]
    ...
    encoder = Encoder(
        ...
        base_model=base_model, num_layers=num_layers
    ).to(device)
    
    # train model for minimize loss
    train_model = TrainModel(encoder, num_hidden, num_proj_hidden, tau).to(device)
    optimizer_train = torch.optim.Adam(
        train_model.parameters(), lr=lr_train, weight_decay=wd_train
    )
    
    # adversarial model for maximize loss
    adv_model = AdversarialModel(
        encoder, num_hidden, num_proj_hidden, num_edge_hidden, tau
    ).to(device)
    optimizer_adv = torch.optim.Adam(
        adv_model.parameters(), lr=lr_adv, weight_decay=wd_adv
    )

    # --- Logging Setup ---
    log_dir = "log"
    os.makedirs(log_dir, exist_ok=True)
    log_file = osp.join(log_dir, f"{args.dataset}.txt")
    ...
    for epoch in range(1, num_epochs + 1):
        # 1. Adversarial Step (Maximize Loss)
        adv_model.train()
        adv_model.zero_grad()
        train_model.eval()
        
        egde_info_adv = adv_model(
            data.x, data.edge_index, torch.ones(data.num_edges).to(device)
        )
        
        upper_edge_weight, lower_edge_weight, reg = generate_aug_edge_weight(egde_info_adv)
        
        z_1 = train_model(
            data.x, data.edge_index, torch.cat([upper_edge_weight, upper_edge_weight], dim=0)
        )  # attacked view 1
        z_2 = train_model(
            data.x, data.edge_index, torch.cat([lower_edge_weight, lower_edge_weight], dim=0)
        )  # attacked view 2
        
        view_loss = train_model.loss(z_1, z_2)
        edge_loss = F.mse_loss(
            egde_info_adv['upper_edge_fea'], egde_info_adv['lower_edge_fea']
        )
        ...
        # Logging
        now = t()
        print(f'(T) | Epoch={epoch:03d}, loss={model_loss:.4f}, '
    ...
    with torch.no_grad():
        ...
        emb_save = emb.cpu().numpy()
        save_dict = {f'embeddings_{args.dataset}': emb_save}
        sio.savemat(f'embeddings_{args.dataset}.mat', save_dict)
        
        labels = data.y.cpu().numpy() 
        save_dict = {f'y_{args.dataset}': labels}
        sio.savemat(f'y_{args.dataset}.mat', save_dict)
